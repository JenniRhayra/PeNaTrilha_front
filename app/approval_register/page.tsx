...
export default function Success_Register() {
    ...
    React.useEffect(() => {
        const path = group == '3' ? 'parkManager_profile' : group == '4' ? 'guide_profile' : 'profile'
        setPath(path)
    }, [group])
    ...
    return (
        <main className="flex flex-col h-screen min-h-screen bg-[#F8F8F8] container mx-auto">
            ...
            <div className='col-span-12 lg:col-span-5 grid place-items-center'>
                <h1 className="text-[#4D5D47] mb-0 text-2xl lg:text-3xl uppercase font-bold text-center mt-5">Enviamos um e-mail para você</h1>
            </div>
            ...
            <div className="text-center">
                <Box
                    ...
                    <div className='mt-1'>
                        <ButtonGreen width='40ch' href='/login'>EDITAR PERFIL</ButtonGreen>
                    </div>
                </Box>
            </div>
        </main>
